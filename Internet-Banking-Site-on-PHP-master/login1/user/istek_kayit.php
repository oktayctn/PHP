...
&nbsp; <a href="logout.php" class="btn btn-danger square-btn-adjust">Çıkış</a> </div>
 ...
 <div class="row">
                     ...
                     <div class="panel panel-default ">
                        <div class="panel-heading">
                           Kredi İsteği  <?php
											 ...
											 //---------kredi isteği yollandı
											
											$istek_isim=$_POST['isim'];
											$istek_soyad=$_POST['soyad'];
											$istek_tckimlik=$_POST['tckimlik'];
											$istek_bankano=$_POST['bankano'];
											$istek_miktar=$_POST['miktar'];
											$istek_mesaj=$_POST['mesaj'];
											
											$sorgu="SELECT tckimlik FROM kredi_istek WHERE tckimlik='$istek_tckimlik'";
											
											$istek_sorgu=mysql_query($sorgu);
											//eğer sistemde bu kullanıcının tc si bulunursa bir daha istek yapamaz
											$istek_kontrol=1;
											while($istek_veri=mysql_fetch_array($istek_sorgu))
											{
											if($istek_tckimlik==$istek_veri['tckimlik'])
											{
											$istek_kontrol=0;  //bu durumda tabloya veri eklenmemeli
											...
											$sql="INSERT INTO kredi_istek (tckimlik,miktar,isim,soyad,bankano,mesaj) VALUES ('$istek_tckimlik','$istek_miktar','$istek_isim','$istek_soyad','$istek_bankano','$istek_mesaj')";
				...
				if($istek_tckimlik==$veri1['tckimlik'] && $istek_isim==$isim && $istek_soyad==$veri2['soyad'] && $istek_bankano==$veri2['banka_id'])
						{
							     if($istek_kontrol)
								       {
				        			mysql_query($sql);
										echo "Talebiniz oluşturuldu. Kısa zamanda değerlendirlecektir.";
										
								       }
						         else
								 		{
										
										echo "Bu kullanıcı daha önce istek yapmıştır. İsteğiniz cevaplanana kadar bekleyiniz.";
										}
						}
					else
						{
						echo "Lütfen kredi talebi için size ait olan bilgileri giriniz..";
						}
				?>
